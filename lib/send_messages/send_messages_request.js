module.exports = function (userId, password, content, recipients, callback, uid, tags) {
    ...
    recipients.forEach(function (recipient, index) {
        ...
        _recipient.attrib.uid = uid || index;
        _recipient.text = recipient;
    });

    if(tags){
        var _tags = subElement(_message, 'ns:tags');
        tags.forEach(function(tag){
            var _tag = subElement(_tags, 'ns:tag');
            _tag.attrib.name = tag.name;
            _tag.attrib.value = tag.value;
        });
    }
